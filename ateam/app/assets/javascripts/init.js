...
/***************** Superfish ******************/
	...
	function addOrRemoveSF(){
		if( window.innerWidth < 1000 && $('body').attr('data-responsive') == '1'){
			...
			$('.slide-out-widget-area-toggle').show();
			...
			$('#mobile-menu').hide();
			$('.slide-out-widget-area-toggle').hide();
...
function morphingOutlines() {
	...
	if($('.morphing-outline').length > 0) {
		...
		$('.morphing-outline').each(function(i){
			...
			$morphingOutlineCSS += '.morphing-outline.instance-'+i+' { padding: '+(30+($width+80 + $border*2 - $height)/2 - $border) +'px 70px}'; //extra space on the outer for mobile/close elements
	...
	function fullWidthRowPaddingAdjustInit(){
		if($('#boxed').length == 0){
			$('.full-width-section, .full-width-content').each(function(){
				...
				var $leftPadding = $(this)[0].style.paddingLeft;
				var $rightPadding = $(this)[0].style.paddingRight;

				if($topPadding.indexOf("%") >= 0) $(this).attr('data-top-percent',$topPadding);
				if($bottomPadding.indexOf("%") >= 0) $(this).attr('data-bottom-percent',$bottomPadding);
				if($leftPadding.indexOf("%") >= 0) $(this).attr('data-left-percent',$leftPadding);
				if($rightPadding.indexOf("%") >= 0) $(this).attr('data-right-percent',$rightPadding);
	...
	function fullWidthRowPaddingAdjustCalc(){
		if($('#boxed').length == 0){
			$('.full-width-section[data-top-percent], .full-width-section[data-bottom-percent], .full-width-content[data-top-percent],  .full-width-content[data-bottom-percent]').each(function(){
				...
				var $leftPadding = ($(this).attr('data-left-percent')) ? $(this).attr('data-left-percent') : 'skip';
				var $rightPadding = ($(this).attr('data-right-percent')) ? $(this).attr('data-right-percent') : 'skip';
				...
				//top
				if($leftPadding != 'skip') {
					$(this).css('padding-left',$windowHeight*(parseInt($leftPadding)/100));
				}

				//bottom
				if($rightPadding != 'skip'){
					$(this).css('padding-right',$windowHeight*(parseInt($rightPadding)/100));
				}
...
$('body').on('click','.slide-out-widget-area-toggle:not(.std-menu) a.closed:not(.animating)',function(){
	...
	/**********HERE***********/
		...
		// NOTE: wonder why there is nothing here?
...
;( function( $, window, undefined ) {
	...
	$.DLMenu.prototype = {
		...
		_config : function() {
			...
			//ADDING BACK BUTTON
			if(this.$el.find('ul.sub-menu').children(".back").length == 0){
...
function condenseHeaderMobileOverride() {
	...
	// TODO: set logoHeight as a function of screen width or height. Or create a get function that has a specific value for the different screen sizes
	// OK for now....
	...
	var shrinkNum = 6;
	var paddingTop = Math.ceil(headerPadding / 1.8);
	var smallNavHeight = $("header#top").height()
	...
	function headerInit() {
		...
		if($('#header-outer[data-small-header="true"]').length > 0){
			...
			// NOTE: ADDITION
			//TODO: Set the padding top and padding bottom to some sort of function of screen height. Instead of all of this garbage
			// OK for now...
			...
			$('header#top nav > ul > li.player-dropdown').stop(true,true).animate({
				...
				'padding-top' :  $(".portal-button").outerHeight()/2
			},{queue:false, duration:250, easing: 'easeOutCubic'});	
		}
		...
		if($('body.material').length == 0) {
			...
			// TODO: check the difference between body.mobile true and false
			if($('body.mobile').length == 0) {
			...
			if($('body.ascend ').length > 0 && $('#header-outer[data-full-width="true"]').length > 0) {
				...
				$('header#top nav > ul.buttons:not(.notif)').css({
	...
	function smallNav(){
		$scrollTriggerOffset = ( $('body.material').length > 0 ) ? 150 : 0;
		var $offset = $(window).scrollTop();
		var $windowWidth = $(window).width();
		var paddingTop = Math.ceil(headerPadding / 1.8);
		var smallNavHeight = $("header#top").height() + paddingTop
		...
		if($offset > $scrollTriggerOffset && $windowWidth > 1000 && $('body.material-ocm-open').length == 0) {
			...
			if($('body.material').length == 0) {
				...
				if($('body.ascend ').length > 0 && $('#header-outer[data-full-width="true"]').length > 0) {
					$('header#top nav > ul.buttons:not(.notif)').stop(true,true).animate({
